...
class RNNForecaster(nn.Module):
    def __init__(self, input_size=1, hidden_size=64, num_layers=1, output_length=1):
        super().__init__()
        self.rnn = nn.LSTM(
    ...
    def forward(self, x):
        ...
        out, _ = self.rnn(x)
        last = out[:, -1, :]          # (B, hidden)
        pred = self.fc(last)          # (B, output_length)
        return pred
...
def rolling_forecast_rnn(
    ...
    while current_T < len(out_df):
        ...
        x = torch.tensor(context.values, dtype=torch.float32)\
