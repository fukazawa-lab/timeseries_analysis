...
class TransformerForecaster(nn.Module):
    ...
    def forward(self, x):
        ...
        last = out[:, -1, :]            # LSTMの last hidden と対応
        pred = self.fc(last)            # (B, output_length)

        return pred

def rolling_forecast_transformer(
    data: pd.DataFrame,
    target_column: str = "target",  # デフォルト値
    T: int = 1600,                  # デフォルト値
    context_length: int = 100,      # デフォルト値
    rolling_step: int = 1,          # デフォルト値

    # ===== Transformer 固有 =====
    d_model: int = 64,              # デフォルト値
    nhead: int = 1,                 # デフォルト値
    num_layers: int = 1,            # デフォルト値
    dim_feedforward: int = 512,     # デフォルト値
